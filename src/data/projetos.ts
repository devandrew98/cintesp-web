 ...
 * Leitura:
 * - admin vê todos;
 * - pesquisadores liberados veem todos os projetos.
 *
 * Escrita:
 * - estrutura/equipe: somente admin;
 * - conteúdo: admin ou responsável;
 * - campos liberados: membros vinculados, conforme configuração do projeto;
 * - chat: admin ou pesquisadores vinculados.
 *
 * As restrições reais de segurança são garantidas pelo RLS do Supabase.
 */
...
function mapProjeto(r: any): Projeto {
  const pesquisadores: PesquisadorVinculado[] = (r.pesquisadores ?? []).map(
    (p: any) => ({
...
const mockProjetos: Projeto[] = [
  {
    ...
    descricao:
      "Protótipo de cadeira de rodas motorizada com desvio automático de obstáculos.",
    status: "em_andamento",
    trl: 5,
    progresso: 55,
    dadosTecnicos:
      "Microcontrolador ESP32, sensores ultrassônicos HC-SR04 (x4), motor DC 24V.",
    ...
    pesquisadores: [
      P("u1", "João Silva", "responsavel"),
      P("u2", "Mariana Souza"),
      P("u4", "Larissa Martins"),
    ],
  ...
  },
  {
    ...
    descricao:
      "App mobile de comunicação aumentativa/alternativa para pessoas não-verbais.",
    status: "planejamento",
    trl: 2,
    progresso: 10,
    dadosTecnicos: "",
    ...
    pesquisadores: [
      P("u3", "Carlos Almeida", "responsavel"),
      P("u6", "Ana Rodrigues"),
    ],
  ...
  },
  {
    ...
    descricao:
      "Órtese funcional para reabilitação, com arquivos de impressão 3D abertos.",
    status: "concluido",
    trl: 8,
    progresso: 100,
    dadosTecnicos:
      "PETG, impressão FDM, tempo de impressão médio 6h por unidade.",
    observacoes:
      "Publicado artigo no congresso regional de tecnologia assistiva.",
    dataInicio: diasNaFrente(-400),
    dataFimPrevista: diasNaFrente(-30),
    responsavelId: "u5",
    responsavelNome: "Rafael Costa",
    pesquisadores: [
      P("u5", "Rafael Costa", "responsavel"),
      P("u1", "João Silva"),
      P("u7", "Lucas Almeida"),
    ],
    camposEditaveisMembros: [
      "trl",
      "progresso",
      "dadosTecnicos",
      "observacoes",
...
const mockMensagens: MensagemProjeto[] = [
  ...
  },
  {
    ...
    corpo:
      "Combinado. Os sensores novos chegam essa semana, já atualizei o progresso.",
...
function vinculado(p: Projeto, usuarioId?: string): boolean {
  return (
    Boolean(usuarioId) && p.pesquisadores.some((v) => v.usuarioId === usuarioId)
  );
}
...
/** Lista projetos: todos os usuários liberados podem visualizar todos. */
...
export async function criarProjeto(input: DadosProjeto): Promise<Projeto> {
  ...
  const vinculos = Array.from(
    new Set([input.responsavelId, ...input.pesquisadoresIds]),
  ).map((usuarioId) => ({
    projeto_id: data.id,
...
export async function atualizarEstruturaProjeto(
  ...
  input: Pick<
    DadosProjeto,
    "responsavelId" | "pesquisadoresIds" | "camposEditaveisMembros"
  >,
): Promise<void> {
  if (USE_MOCK || !supabase) {
    ...
    const membros = input.pesquisadoresIds.filter(
      (uid) => uid !== input.responsavelId,
    );
    p.pesquisadores = [
      {
        usuarioId: input.responsavelId,
 ...
 */
export async function atualizarConteudoProjeto(
  id: string,
  patch: ConteudoProjeto,
): Promise<void> {
  if (USE_MOCK || !supabase) {
    const p = mockProjetos.find((x) => x.id === id);
    if (!p) throw new Error("Projeto não encontrado.");
  ...
  if (patch.descricao !== undefined)
    payload.descricao = patch.descricao.trim() || null;
  if (patch.status !== undefined) payload.status = patch.status;
  if (patch.dataInicio !== undefined)
    payload.data_inicio = patch.dataInicio || null;
  if (patch.dataFimPrevista !== undefined)
    payload.data_fim_prevista = patch.dataFimPrevista || null;
  if (patch.trl !== undefined) payload.trl = patch.trl;
  if (patch.progresso !== undefined) payload.progresso = patch.progresso;
  if (patch.dadosTecnicos !== undefined)
    payload.dados_tecnicos = patch.dadosTecnicos.trim() || null;
...
export async function listarMensagensProjeto(
  projetoId: string,
): Promise<MensagemProjeto[]> {
  if (USE_MOCK || !supabase)
    return mockMensagens.filter((m) => m.projetoId === projetoId);
  const { data, error } = await supabase
    .from("projeto_mensagens")
    .select(
      "id, projeto_id, autor_id, corpo, anexo_url, anexo_nome, anexo_tipo, anexo_tamanho, created_at, autor:autor_id (nome, foto_url)",
    )
...
export async function enviarMensagemProjeto(
  projetoId: string,
  msg: NovaMensagemProjeto,
): Promise<void> {
  if (USE_MOCK || !supabase) {
...
export async function enviarAnexoProjeto(
  projetoId: string,
  arquivo: File,
