  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        >
          {isLoading ? (
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {recentes.map((a) => (
                <AvisoRecenteCard key={a.id} aviso={a} />
              ))}
            </div>
          )}
        </SectionCard>
        ...
        >
          {destaques.length === 0 ? (
            ...
            destaques.map((a) => <AvisoDestaqueCard key={a.id} aviso={a} />)
          )}
        </SectionCard>
      </div>
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 rounded-2xl border border-brand-100 bg-brand-50 p-5 dark:border-brand-500/20 dark:bg-brand-500/10 sm:flex-row sm:items-center">
        ...
        <Button variant="secondary">Configurar notificações</Button>
      </div>
...
function AvisoRecenteCard({ aviso }: { aviso: Aviso }) {
  const info = tipoAvisoInfo[aviso.tipo]
  const status = statusAvisoInfo[aviso.status]
  const Icon = info.icon

  return (
    <div className="flex gap-3 rounded-xl px-3 py-3.5 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/60">
...
function AvisoDestaqueCard({ aviso }: { aviso: Aviso }) {
  const info = tipoAvisoInfo[aviso.tipo]
  return (
    <div className="rounded-xl border border-slate-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-800/40">
