...
export function ProjetoDetailModal({
  ...
  const souResponsavel = Boolean(
    projeto && perfil?.id === projeto.responsavelId,
  );
  const souMembro = Boolean(
    projeto &&
    perfil?.id &&
    projeto.pesquisadores.some((v) => v.usuarioId === perfil.id),
  );
  const podeConteudoTotal = ehAdmin || souResponsavel;

  const podeVerProjeto = true;

  const podeVerChat = ehAdmin || souMembro;
  ...
  const podeEditarAlgumConteudo =
    podeConteudoTotal ||
    ["trl", "progresso", "dadosTecnicos", "descricao", "observacoes"].some(
      (c) => podeEditarCampo(c as CampoEditavelProjeto),
    );
  ...
  function submeterConteudo() {
    ...
    if (podeEditarCampo("trl"))
      patch.trl = trl ? (Number(trl) as TRL) : undefined;
  ...
  const [membrosIds, setMembrosIds] = useState<string[]>(
    (projeto?.pesquisadores ?? [])
      .filter((p) => p.papel === "membro")
      .map((p) => p.usuarioId),
  );
  const [camposLiberados, setCamposLiberados] = useState<
    CampoEditavelProjeto[]
  >(projeto?.camposEditaveisMembros ?? []);

  const salvarEquipe = useMutation({
    mutationFn: () =>
      atualizarEstruturaProjeto(projeto!.id, {
        responsavelId,
  ...
  const { data: mensagens = [], isLoading: carregandoChat } = useQuery({
    ...
    enabled: Boolean(projeto) && podeVerChat,
    refetchInterval: projeto && podeVerChat ? 4000 : false,
  });

  const enviar = useMutation({
    mutationFn: (corpo: string) =>
      enviarMensagemProjeto(projeto!.id, { corpo }),
    onSuccess: () => {
      setTexto("");
    ...
    >
      <div className="space-y-6">
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            tone={st.tone as "green" | "amber" | "blue" | "red" | "slate"}
            dot
          >
            {st.label}
          </Badge>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {projeto.pesquisadores.map((p) => (
            <span
              key={p.usuarioId}
              className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 py-1 pl-1 pr-2.5 text-xs dark:bg-slate-800"
            >
              <Avatar
                nome={p.nome ?? "—"}
                fotoUrl={p.fotoUrl}
        ...
        {editandoEquipe && (
          <div className="space-y-4 rounded-xl border border-slate-200 p-4 dark:border-slate-700">
            <EquipeCamposForm
              ...
              onAlternarCampo={(c) =>
                setCamposLiberados((a) =>
                  a.includes(c) ? a.filter((x) => x !== c) : [...a, c],
                )
              }
            />
            <Button
              onClick={() => salvarEquipe.mutate()}
              disabled={!responsavelId || salvarEquipe.isPending}
            >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Início">
            {podeConteudoTotal ? (
              ...
              <p className="inline-flex items-center gap-1.5 text-sm text-slate-700 dark:text-slate-200">
                ...
                {formatDateOnlyBR(projeto.dataInicio)}
              </p>
            )}
          </Field>
          <Field label="Fim previsto">
            {podeConteudoTotal ? (
              ...
              <p className="inline-flex items-center gap-1.5 text-sm text-slate-700 dark:text-slate-200">
                <Calendar className="h-3.5 w-3.5 text-slate-400" />{" "}
                {formatDateOnlyBR(projeto.dataFimPrevista)}
              </p>
            )}
          </Field>
          ...
          <Field label="TRL (maturidade tecnológica)">
            {podeEditarCampo("trl") ? (
              ...
              <p className="text-sm text-slate-700 dark:text-slate-200">
                {rotuloTRL(projeto.trl)}
              </p>
            )}
          </Field>
          <Field
            label={`Progresso — ${podeEditarCampo("progresso") ? progresso : projeto.progresso}%`}
          >
            ...
            <ProgressoBar
              progresso={
                podeEditarCampo("progresso") ? progresso : projeto.progresso
              }
              className="mt-2"
            />
          </Field>
          ...
          <Field
            ...
            hint={
              !podeEditarCampo("descricao")
                ? undefined
                : "Objetivo, escopo, contexto do projeto."
            }
          >
            {podeEditarCampo("descricao") ? (
              <Textarea
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {salvarConteudo.isError && (
              <span className="text-xs text-red-500">
                {mensagemErro(salvarConteudo.error)}
              </span>
            )}
            {salvarConteudo.isSuccess && (
              <span className="text-xs text-brand-600">Salvo!</span>
            )}
          </div>
        )}
              ...
              {carregandoChat ? (
                ...
                mensagens.map((m) => {
                  ...
                  return (
                    <div
                      key={m.id}
                      className={cn("flex gap-2", minha && "flex-row-reverse")}
                    >
                      <Avatar
                        nome={m.autorNome ?? "—"}
